...
- **`apphashes.json`** — the agent's own SHA256 cache for installed-app executables (`apphashcache_windows.go`), keyed by resolved exe path (AppX manifest's `Executable` attribute, or the registry Uninstall key's `DisplayIcon` value when it points at a real `.exe`; never populated for winget-only entries, which have no resolvable path) so a binary already hashed in a prior cycle isn't re-read off disk unless its size or mtime changed. Plain overwrite, same non-atomic write pattern as `status.json` — never read by the tray, agent-internal only. Feeds SOAR's Binary Integrity feature (`backend/docs/settings.md#binary-integrity`): VirusTotal file-reputation lookup to flag sideloaded/tampered binaries.
