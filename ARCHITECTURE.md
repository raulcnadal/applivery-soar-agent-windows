...
- **`card.go`** — the status card: a second borderless top-level window (`WS_POPUP`, not a dialog resource), custom-painted to match the SOAR web dashboard's own Workspace Profile modal styling (banner logo, device name/workspace slug, status pills, boxed sections) so the native tray experience visually reads as the same product as the web app. `enableAcrylicBackdrop` gives it a translucent, blurred backdrop — the Windows equivalent of the macOS menu bar app's `NSVisualEffectView` — via `DwmExtendFrameIntoClientArea` (documented) marking the whole client area as DWM-composited glass, where pure `RGB(0,0,0)` becomes see-through, plus `SetWindowCompositionAttribute` (undocumented, but the only way to get true Acrylic on Windows 10/11 — no public alternative exists) to set the tint/opacity. Needs Windows 10 1803+ and DWM composition on; fails closed to the original fully-opaque fill everywhere else, both checked at runtime before the card ever paints.
