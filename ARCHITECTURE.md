...
- **`card.go`** — the status card: a second borderless top-level window (`WS_POPUP`, not a dialog resource), custom-painted to match the SOAR web dashboard's own Workspace Profile modal styling (banner logo, device name/workspace slug, status pills, boxed sections) so the native tray experience visually reads as the same product as the web app.
