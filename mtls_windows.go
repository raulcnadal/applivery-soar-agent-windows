...
func ensureMtlsIdentity() {
	...
	if id == nil {
		...
		if config.EnrollmentSecret != "" {
			ensureSelfServiceEnrollment(baseURL, config)
			return
		}
		return // no certificate yet and nothing to register with — stay on legacy auth this cycle
	}
...
//
// The alternative to BootstrapToken-based registration above, for fleets
// with no per-device provisioning step to slot a unique token into.
// EnrollmentSecret is the SAME value on every device (pushed via one static
// Managed Configuration policy, unlike BootstrapToken), so the backend
// additionally checks the claimed serial number against Applivery's own
// live device list before issuing anything — see
// mtlsEnrollment.service.ts's module doc (backend repo) for the full
// security model this trades off against the per-device token path. Only
// takes effect when BootstrapToken is empty (ensureMtlsIdentity above) —
// a device with both configured always prefers the per-device token.
var (
	selfServiceEnrollMu      sync.Mutex
	selfServiceEnrollPending bool
	selfServiceEnrollPrivKey *ecdsa.PrivateKey // held across cycles while a request is pending, so a later poll finalizes with the SAME keypair the original CSR was generated for
)

// ensureSelfServiceEnrollment is the per-cycle entry point: POST the
// request once, then switch to polling on every subsequent cycle until the
// backend either issues a certificate (silent mode, or an admin approved
// it) or rejects the request (at which point the next cycle submits a
// fresh request — matches the backend's own "a new request after a
// decision is a fresh row" design).
func ensureSelfServiceEnrollment(baseURL *url.URL, config Config) {
	selfServiceEnrollMu.Lock()
	pending := selfServiceEnrollPending
	privKey := selfServiceEnrollPrivKey
	selfServiceEnrollMu.Unlock()

	if pending && privKey != nil {
		pollSelfServiceEnrollment(baseURL, config, privKey)
		return
	}
	requestSelfServiceEnrollment(baseURL, config)
}

// requestSelfServiceEnrollment — POST /api/device-mtls/enroll, authenticated
// via the shared X-Enrollment-Secret header (never a client cert — this
// device doesn't have one yet). A 200 means the backend issued a
// certificate immediately (silent mode); a 202 means it's queued for admin
// approval, and this same CSR/keypair is remembered for the poll that
// follows on later cycles — the CSR sent here is exactly what an admin's
// Approve click ultimately signs.
func requestSelfServiceEnrollment(baseURL *url.URL, config Config) {
	serialNumber := GetSerialNumber()
	if !isUsableSerial(serialNumber) {
		log.Println("mTLS: cannot self-service enroll without a usable device serial number — will retry next cycle.")
		return
	}

	privKey, csrPem, err := generateMtlsKeyAndCsr(serialNumber)
	if err != nil {
		log.Printf("mTLS: failed to generate key/CSR for self-service enrollment: %v", err)
		return
	}

	enrollURL := baseURL.ResolveReference(&url.URL{Path: "/api/device-mtls/enroll"}).String()
	body, err := json.Marshal(map[string]string{"csrPem": csrPem, "serialNumber": serialNumber, "platform": "windows"})
	if err != nil {
		log.Printf("mTLS: failed to marshal self-service enrollment payload: %v", err)
		return
	}

	req, err := http.NewRequest("POST", enrollURL, bytes.NewReader(body))
	if err != nil {
		log.Printf("mTLS: could not build self-service enrollment request: %v", err)
		return
	}
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("X-Workspace-Slug", config.WorkspaceSlug)
	req.Header.Set("X-Enrollment-Secret", config.EnrollmentSecret)

	client := &http.Client{Timeout: 30 * time.Second}
	resp, err := client.Do(req)
	if err != nil {
		log.Printf("mTLS: self-service enrollment request failed: %v — will retry next cycle.", err)
		return
	}
	defer resp.Body.Close()
	...
	case http.StatusOK:
		var result mtlsIssueResponse
		if err := json.NewDecoder(resp.Body).Decode(&result); err != nil {
			log.Printf("mTLS: could not parse self-service enrollment response: %v", err)
			return
		}
		if err := finalizeIssuedIdentity(privKey, result); err != nil {
			log.Printf("mTLS: failed to store issued certificate: %v", err)
		...
		log.Println("mTLS: self-service enrollment issued immediately (silent mode) — this device now authenticates to SOAR via client certificate.")
	case http.StatusAccepted:
		selfServiceEnrollMu.Lock()
		selfServiceEnrollPending = true
		selfServiceEnrollPrivKey = privKey
		selfServiceEnrollMu.Unlock()
		log.Println("mTLS: self-service enrollment request submitted — awaiting admin approval, will check again next cycle.")
	default:
		log.Printf("mTLS: self-service enrollment rejected by backend (HTTP %d) — will retry next cycle.", resp.StatusCode)
	}
}

// pollSelfServiceEnrollment — GET /api/device-mtls/enroll/status. Same
// X-Enrollment-Secret auth as the initial request, re-checked by the
// backend on every single poll.
func pollSelfServiceEnrollment(baseURL *url.URL, config Config, privKey *ecdsa.PrivateKey) {
	serialNumber := GetSerialNumber()
	if !isUsableSerial(serialNumber) {
		return
	}

	statusURL := baseURL.ResolveReference(&url.URL{Path: "/api/device-mtls/enroll/status", RawQuery: url.Values{"serialNumber": {serialNumber}}.Encode()}).String()
	req, err := http.NewRequest("GET", statusURL, nil)
	if err != nil {
		log.Printf("mTLS: could not build self-service enrollment poll request: %v", err)
		return
	}
	req.Header.Set("X-Workspace-Slug", config.WorkspaceSlug)
	req.Header.Set("X-Enrollment-Secret", config.EnrollmentSecret)
	...
	resp, err := client.Do(req)
	if err != nil {
		log.Printf("mTLS: self-service enrollment poll failed: %v — will retry next cycle.", err)
		return
	}
	defer resp.Body.Close()
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		log.Printf("mTLS: self-service enrollment poll rejected by backend (HTTP %d) — will retry next cycle.", resp.StatusCode)
		return
	}
	...
	if err := json.NewDecoder(resp.Body).Decode(&result); err != nil {
		log.Printf("mTLS: could not parse self-service enrollment poll response: %v", err)
		return
	}

	switch result.Status {
	case "issued":
		if err := finalizeIssuedIdentity(privKey, mtlsIssueResponse{CertPem: result.CertPem, CaCertPem: result.CaCertPem, NotAfter: result.NotAfter}); err != nil {
			log.Printf("mTLS: failed to store approved certificate: %v", err)
			return
		}
		selfServiceEnrollMu.Lock()
		selfServiceEnrollPending = false
		selfServiceEnrollPrivKey = nil
		selfServiceEnrollMu.Unlock()
		log.Println("mTLS: self-service enrollment approved — this device now authenticates to SOAR via client certificate.")
	case "rejected":
		selfServiceEnrollMu.Lock()
		selfServiceEnrollPending = false
		selfServiceEnrollPrivKey = nil
		selfServiceEnrollMu.Unlock()
		log.Printf("mTLS: self-service enrollment request was rejected by an admin (%s) — will submit a fresh request next cycle.", result.Reason)
	default: // "pending"
		log.Println("mTLS: self-service enrollment still awaiting admin approval.")
	}
}
